...
// TestExplainVersionConsistency verifies all explain.txt files have the same ClickHouse version header.
// This ensures test expectations were generated from a consistent ClickHouse version.
func TestExplainVersionConsistency(t *testing.T) {
	...
	versionPrefix := "-- Generated by ClickHouse "
	var expectedVersion string
	var filesWithVersion []string
	var filesWithoutVersion []string
	versionMismatches := make(map[string][]string) // version -> list of files
	...
	for _, entry := range entries {
		...
		// Find all explain*.txt files in this test directory
		explainFiles, err := filepath.Glob(filepath.Join(testDir, "explain*.txt"))
		if err != nil {
			continue
		}

		for _, explainFile := range explainFiles {
			...
			if !strings.HasPrefix(firstLine, versionPrefix) {
				filesWithoutVersion = append(filesWithoutVersion, explainFile)
				continue
			}

			version := strings.TrimPrefix(firstLine, versionPrefix)
			filesWithVersion = append(filesWithVersion, explainFile)

			if expectedVersion == "" {
				expectedVersion = version
			}

			versionMismatches[version] = append(versionMismatches[version], explainFile)
		}
	}

	// Report files without version headers
	if len(filesWithoutVersion) > 0 {
		t.Errorf("Found %d explain files without version header (expected '%s...'):",
			len(filesWithoutVersion), versionPrefix)
		// Show first 10 files
		for i, f := range filesWithoutVersion {
			if i >= 10 {
				t.Errorf("  ... and %d more", len(filesWithoutVersion)-10)
				break
			}
			t.Errorf("  %s", f)
		}
	...
	if len(versionMismatches) > 1 {
		t.Errorf("Found %d different ClickHouse versions in explain files (expected all to be the same):", len(versionMismatches))
		for version, files := range versionMismatches {
			t.Errorf("  Version %s: %d files", version, len(files))
		}
	}

	// Log summary
	if len(filesWithVersion) > 0 && len(versionMismatches) == 1 {
		t.Logf("All %d explain files have consistent version: %s", len(filesWithVersion), expectedVersion)
	}
}
